...
def simulate(output_dir, lambda_12_val):
    ...
    snapshot_interval_steps = 42

    h5_path = output_dir / "snapshots.h5"
    ...
    dset_pos = h5file.create_dataset(
        "lynx_pos",
        shape=(num_snapshots,),
        dtype=vlen_float
    )
    ...
    for step in range(STEPS):
        ...
        reactions = [
            ...
            lambda: lynx_movement_reaction(pos, state, params, B, lambda_12)
        ]
        ...
        print(t, B.sum(), len(pos))

        history.append((t, B.sum(), n_state1, n_state2))

        # -------------------------------------------------
        # SAVE SNAPSHOT EVERY 42 STEPS
        # -------------------------------------------------
        ...
        if step % snapshot_interval_steps == 0:
            ...
            dset_B[snapshot_idx] = B.astype(np.int32)
