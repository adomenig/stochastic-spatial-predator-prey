...
for(n in 1:length(SNs)) {
  # Extract numeric CTN from the filename
  ctn <- sub("^(\\d+).*", "\\1", basename(SNs[n]))
  
  LYNX[[ctn]] <- with(
    read.csv(skip = 22, SNs[n], header = TRUE, na.strings = c("", NA)),
    data.frame(
      Time = round_date(ymd_hms(GPS.Fix.Time), unit = "hour"),
      Long = GPS.Longitude, Lat = GPS.Latitude,
      Type = GPS.Fix.Attempt,
      Easting = GPS.UTM.Easting, Northing = GPS.UTM.Northing,
      Activity = c(Activity.Count[-1], NA),
      Mortality = c(Mortality[-1], 'NA')
    )
  ) %>%
    mutate(CTN = ctn) %>%
    filter(Type %in% c("Resolved QFP", "Uncertain QFP")) %>%
    dplyr::select(CTN, everything()) %>%
    mutate(TimeDiff = c(NA, round(diff(as.numeric(Time))/3600)),
           Mortality = as.factor(ifelse(Mortality == "Yes", "2",
...
LYNX <- LYNX %>% map(function(df){
  df$CTN <- gsub("^/", "", df$CTN)  # remove leading slash
  df
})
...
output_file <- file.path(output_path, "lynx_initial_clean.df")
...
# save the CSV
write.csv(lynx.df, output_file, row.names = FALSE)
